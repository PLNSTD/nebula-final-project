...
def insert(country, year_pop, population, density_per_km2, pop_rank, density_rank):
    ...
    try:
        cur = conn.cursor()
        query = """
        INSERT INTO projections_population (country, population_year, population, density_per_km2, population_rank, density_rank)
        VALUES (%s, %s, %s, %s, %s, %s)
...
def get_by_country(country):
    ...
    try:
        cur = conn.cursor()
        query = """
        SELECT * FROM projections_population WHERE country = %s;
        """
